...
def trapezoid_integral(func: Union[str, Callable],
                       a :np.float64,
...
def multiple_trapezoid_integral(func: Union[str, Callable],
                                a :np.float64,
                                b :np.float64,
                                n :int) -> np.float64:
    if n < 1:
...
def simpson13_integral(func: Union[str, Callable],
                       a :np.float64,
                       b :np.float64) -> np.float64:
    mid_point :np.float64 = (b + a) / 2
...
def multiple_simpson13_integral(func: Union[str, Callable],
                                a :np.float64,
                                b :np.float64,
                                n :int) -> np.float64:
    if n < 1:
    ...
    h :np.float64 = (b - a) / n
    points :np.float64 = np.arange(a, b + h, h)
...
def simpson38_integral(func: Union[str, Callable],
                       a :np.float64,
                       b :np.float64) -> np.float64:
    h :np.float64 = (b - a) / 3.0
...
def richards_extrapolation(func: Union[str, Callable],
                           a :np.float64,
                           b :np.float64,
                           h1 :np.float64,
                           h2 :np.float64) -> np.float64:
    i_h1 = multiple_trapezoid_integral(func, a, b, h1)
    i_h2 = multiple_trapezoid_integral(func, a, b, h2)
...
def gauss_quadrature(func: str,
    ...
    final_result :np.array = np.zeros((x.shape[0], 1),
                                      dtype=np.float64)
